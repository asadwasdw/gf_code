    ...
    Point pointRotate(Point a,Point b) {
        Vector ab = a-b;
        // ab.print();
        ab = Rotate(ab,pi/2);
        // ab.print();
    ...
    double js(Vector a) {
        double t = acos(a.x/Len(a))/pi*180;
        if(a.y<0)t = -t;
        while(t<0)t+=360;
        return t;
    }
    ...
    double f(double i) {
        Point res = c;
        res.x += cos(i*pi/180)*r;
        res.y += sin(i*pi/180)*r;
    ...
    void solve()
    {
        ...
        if(a.x == b.x  && a.y == b.y) {
            if(Distance(a,c) <= r) cout <<'0'<<endl;
            else cout << 2*(Distance(a,c))<<endl;
            return ;
        }

        if(r == 0) {
            cout << Distance(a,c) + Distance(b,c) << endl;
            return ;
        }

        if(pd(Distance(a,c),r) || pd(Distance(b,c),r) ) {
            cout << Distance(a,b) << endl ;
            return ;
        }

        Line AB(a,b);
        if( pd(Dis_point_seg(c,AB), r)) {
            cout << Distance(a,b) << endl ;
            return ;
        }
        ...
        double ans = INF;
        {
            ...
            while(r-l > eps) {
                ...
                // cerr << mid1  << ' ' << mid2 << '-';
                // cerr << f(mid1)  << ' ' << f(mid2) << '\n';
                if(sgn (f(mid1) - f(mid2)) == 0) l = mid1,r=mid2;
