...
void build(int root,int la,int lb,int &len){
    ...
    if(a[la] == b[lb]){
        ...
        build(ls_root,ls_la,ls_lb,ls_len);

        if(ls_len + 1 < len){
            int rs_la = la + ls_len + 2;
            int rs_lb = lb + ls_len + 2;
            int rs_root = a[la+ls_len+1];
        ...
        int rs_root = b[lb];
        int rs_la = pa[b[lb]]+1,rs_lb = lb+1,rs_rb = pb[a[la]]-1;
        int rs_len = rs_rb - rs_lb + 1;
        build(rs_root,rs_la,rs_lb,rs_len);

        pre[ls_root] = pre[rs_root] = root;
        len = rs_len + ls_len + 2;
    }
