...
void solve()
{
    ...
    vector<int> T;
    for(int i = 1; i <= n; i ++) T.push_back(L[fp[i]]); dfn.init(T  );T.clear();
    for(int i = 1; i <= n; i ++) T.push_back(R[fp[i]]);fdfn.init(T);T.clear();
    ...
    while(q --) {
        ...
        if(op == 1) {
            ...
            {
                int idx = L[p[x]], idy = L[p[y]];
                swap(L[p[x]], L[p[y]]);
                dfn.modify(1, p[x], L[p[x]]);
                dfn.modify(1, p[y], L[p[y]]);

                swap(fL[idx], fL[idy]);
            }

            {
                int idx = R[p[x]], idy = R[p[y]];
                swap(R[p[x]], R[p[y]]);
                swap(fR[idx], fR[idy]);
                fdfn.modify(1, p[x], R[p[x]]);
                fdfn.modify(1, p[y], R[p[y]]);
            }
            ...
            // for(int i = 1; i <= 2*n; i ++) cout << fR[i] << " "; cout << endl;
