...
bool three_point_line(Point a,Point b, Point c) {
...
void solve()
{
    ...
    for(int i = 0; i < n; i ++) {
        for(int j = 0;j < n;j ++) {
            ...
            for(int k = 0; k < n; k ++) {
                if(sgn(Dis_point_line(p[k],line)) == 0) {
                    d[i][j].push_back(k);
                    // cout << k << " ";
               }
            } 
    ...
    for(int st = 0; st < (1<<n) ;st ++ ) {
        ...
        for(int i = 0;i < n; i++) {
            ...
            for(int j = 0; j < n; j++) {
                if((st>>(j)) & 1)continue;
                int xdst = st;
                for(auto t:d[i][j]) xdst|=(1<<t);
                // dp[xdst] = min(dp[xdst],dp[st] + 1);
                if(dp[xdst] > dp[st] + 1) {
                    // cout << xdst <<"<----"<<st<<" "<<i<<" "<<j<<endl;
                    pre[xdst] = st;
                    dp[xdst] = dp[st] + 1;
                }
            }
    ...
    // for(int i=0;i<(1<<n);i++)cout <<dp[i]<<" ",print(i);
