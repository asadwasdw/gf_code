...
void solve()
{
    ...
    for(int i = 1; i <= n; i ++) {
        ...
        id[i][1] = newnode();
        cin >> v[i];
        bound_mcmf.adde(id[i][0], id[i][1], v[i], v[i], 0);
        bound_mcmf.adde(S, id[i][0], 0, INF, 0);
        bound_mcmf.adde(id[i][1], t, 0, INF, 0);
    }

    T = newnode(); 

    bound_mcmf.adde(t, T, m, m, 0);
    ...
    for(int i = 1; i < n; i ++) {
        for(int j = 1; j <= n - i; j ++) {
            ...
            int x; cin >> x;
            if(x == -1) continue;
            bound_mcmf.adde(id[u][1], id[v][0], 0, INF, x);
        }
    }
